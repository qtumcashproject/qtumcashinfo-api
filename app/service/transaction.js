...
class TransactionService extends Service {
  async getTransaction(id) {
    ...
    const {Address: RawAddress} = this.app.qtumcashinfo.lib
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.qtumcashinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.qtumcashinfo.rpc(this.app.config.qtumcashinfo.rpc)
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.qtumcashinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.qtumcashinfo.lib
  ...
  async transformQRC20Transfers(outputs) {
    const TransferABI = this.app.qtumcashinfo.lib.Solidity.qrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
  ...
  async transformQRC20UnconfirmedTransfers(outputs) {
    const {OutputScript, Solidity} = this.app.qtumcashinfo.lib
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.qtumcashinfo.lib
    const {
